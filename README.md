...
コピー先:
- `processing/renamed/*.pdf` → `export/files/<category>/`
- `processing/metadata-ready/*.metadata.json` → `export/metadata/<category>/`
...
| `export_utils.py` | export v2 共通ユーティリティ（category抽出・パス生成・NFC正規化） | 内部モジュール |
| `migrate_export_v2.py` | 既存 export/ フラット構造 → category サブディレクトリ（v2）への移行 | 初回移行時 |
| `export_to_dropbox.py` | renamed/ と metadata-ready/ を export/<category>/ へコピー（v2構造） | check 後 |

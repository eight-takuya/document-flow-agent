...
## process_inbox.py の動作方針

| オプション | 動作 | ファイルへの影響 |
|---|---|---|
| `--dry-run`（デフォルト） | 分析・表示のみ | なし（読み取りのみ） |
| `--apply` | renamed/ へ safe copy + metadata 生成 | コピーのみ（inbox は残る） |

### --apply の安全制約
- `processing/ocr-error/` に分類されたファイルは **処理対象外**
- `processing/review-required/` に分類されたファイルは **処理対象外**
- 同名ファイルが存在する場合は **上書きせず** `-001`, `-002` の連番を付与
- **inbox の元ファイルは削除しない**
- `--apply` 後も category は空のまま → 人間が手動で補完する

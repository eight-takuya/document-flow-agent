...
GARBLED_PATTERN = re.compile(
    # OCR誤認識で出やすい特殊漢字セット（実際に観測された文字化けパターンに基づく）
    r"[醇珍嘗叡蕊燕蕎墨廃鐸髄軋餉帷桝燐]"
    # 数字の直後に記号が続く異常パターン（ダッシュ類・波ダッシュは正常なので除外）
    r"|\d[^\w\-_\.\s　-鿿＀-￯‐-―−〜]"
    # 除去: ひらがな+カタカナ混在ルール（正常な日本語で頻発するため誤検知が多い）
    # 除去: 非ASCII20文字超ルール（長い正常な日本語タイトルでも誤検知するため）
)
